...
// SlotSink aggregates events per Ethereum slot and exports
// via OTLP on slot boundaries.
...
func (s *SlotSink) Start(ctx context.Context) error {
	...
	s.meter = s.exporter.MeterProvider().Meter("observoor.slot")
...
func (s *SlotSink) exportSnapshot(snap BucketSnapshot) {
	...
	}).Debug("Exported slot snapshot")
}

//nolint:cyclop // instrument creation is inherently sequential
func (s *SlotSink) createInstruments() error {
	var err error
